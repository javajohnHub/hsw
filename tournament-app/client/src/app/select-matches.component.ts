  ...
  template: `
    <div class="wheel-bg">
      <h1 class="wheel-title">
        Current Week: {{ currentWeek }} / {{ maxWeeks }}
      </h1>
      <div class="wheel-main-layout">
        <div class="wheel-left-section">
          ...
          <div class="wheel-btn-row">
            <button class="wheel-btn" (click)="openStatsModal()">
              View Matchup Stats
            </button>
          </div>
        </div>
        <div class="wheel-right-section">
          <div class="selected-player animate-pop">
            ...
            <div class="selected-name">
              {{ selectedPlayer || "No selection yet" }}
            </div>
            <div *ngIf="lastMatch" class="match-feedback">
              Last match: {{ lastMatch.player1 }} vs {{ lastMatch.player2 }}
            </div>
            <div *ngIf="wheelFeedback" class="match-feedback">
              {{ wheelFeedback }}
            </div>
          </div>
          <div *ngIf="showGeneratedMsg" class="match-feedback">
            Round robin matches generated and saved!
          </div>
            ...
            <div class="matchup-grid">
              ...
              >
                ...
                >
                  <span
                    *ngIf="
                      getMatchupCount(matchup.player1, matchup.player2) === 1
                    "
                    class="new-matchup"
                    >NEW!</span
                  >
                  <span
                    *ngIf="
                      getMatchupCount(matchup.player1, matchup.player2) > 1
                    "
                    class="repeat-matchup"
                  >
                    {{ getMatchupCount(matchup.player1, matchup.player2)
                    }}{{
                      getOrdinalSuffix(
                        getMatchupCount(matchup.player1, matchup.player2)
                      )
  ...
  styles: [
    `
      ...
      .tv-static {
        ...
        background-size: 40px 40px, 60px 60px, 30px 30px, 50px 50px, 45px 45px,
          35px 35px, 25px 25px, 55px 55px;
      ...
      @keyframes staticNoise {
        0%,
        100% {
          background-position: 0 0, 20px 10px, 40px 30px, 10px 50px, 30px 20px,
            50px 40px, 15px 15px, 35px 25px;
        }
        12.5% {
          background-position: 5px 5px, 25px 15px, 45px 35px, 15px 55px,
            35px 25px, 55px 45px, 20px 20px, 40px 30px;
        }
        25% {
          background-position: 10px 10px, 30px 20px, 50px 40px, 20px 60px,
            40px 30px, 60px 50px, 25px 25px, 45px 35px;
        }
        37.5% {
          background-position: 15px 15px, 35px 25px, 55px 45px, 25px 65px,
            45px 35px, 65px 55px, 30px 30px, 50px 40px;
        }
        50% {
          background-position: 20px 20px, 40px 30px, 60px 50px, 30px 70px,
            50px 40px, 70px 60px, 35px 35px, 55px 45px;
        }
        62.5% {
          background-position: 25px 25px, 45px 35px, 65px 55px, 35px 75px,
            55px 45px, 75px 65px, 40px 40px, 60px 50px;
        }
        75% {
          background-position: 30px 30px, 50px 40px, 70px 60px, 40px 80px,
            60px 50px, 80px 70px, 45px 45px, 65px 55px;
        }
        87.5% {
          background-position: 35px 35px, 55px 45px, 75px 65px, 45px 85px,
            65px 55px, 85px 75px, 50px 50px, 70px 60px;
        }
      }
      .tv-label {
        margin-top: 8px;
...
export class SelectMatchesComponent
  implements AfterViewInit, OnDestroy, OnInit
{
  ...
  wheelCanvasRef!: ElementRef<HTMLCanvasElement>;
  ...
  private readonly RAWG_BASE_URL = "/media";
  ...
  resizeWheelCanvas() {
    try {
      ...
      const w = Math.max(
        200,
        Math.min(Math.floor(parentRect.width), maxAllowed)
      );
      const h = Math.max(
        200,
        Math.min(Math.floor(parentRect.height || parentRect.width), maxAllowed)
      );
  ...
  animateStatic() {
    ...
    const animate = () => {
      ...
      const isSelectionComplete = this.isWeekSelectionComplete(
        this.currentWeek
      );
  ...
  drawWheel(playerList?: string[]) {
    const canvas =
      (this.wheelCanvasRef && this.wheelCanvasRef.nativeElement) ||
      (document.querySelector("canvas.wheel-canvas") as HTMLCanvasElement);
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (name.length > 9) {
        ...
        const truncatedSecond = this.truncateText(
          ctx,
          secondLine,
          maxTextWidth
        );
  ...
  generateWeeklyMatches() {
    ...
    if (activePlayers.length % 2 === 0) {
      ...
      const playersForMatches = activePlayers.filter(
        (p) => p !== playerWithBye
      );
    ...
    const otherWeekMatches = this.roundRobinMatches.filter(
  ...
  getPlayerForBye(): string {
    ...
    const playersWithMinByes = activePlayers.filter(
      (player) => byeCounts[player] === minByes
    );

    // Return random player from those with minimum byes
    return playersWithMinByes[
      Math.floor(Math.random() * playersWithMinByes.length)
    ];
  }
  ...
  openStatsModal() {
    ...
    activePlayers.forEach((player) => {
      ...
      statsMessage += `  Unique opponents: ${uniqueOpponents.length}/${
      ...
      if (uniqueOpponents.length > 0) {
        ...
        uniqueOpponents.forEach((opponent) => {
          ...
          statsMessage += `    ${opponent}: ${matchCount} time${
            matchCount > 1 ? "s" : ""
          }\n`;
        });
      }
    ...
    statsMessage += `Fairness: ${Math.round(
      (actualUniqueMatchups / totalPossibleMatchups) * 100
    )}%\n`;
  ...
  saveMatches() {
    localStorage.setItem(
      "rnd_roundRobinMatches",
      JSON.stringify(this.roundRobinMatches)
    );
  }
  ...
  isWeekSelectionComplete(week: number): boolean {
    ...
    const expectedMatchCount =
      activePlayers.length % 2 === 1 ? expectedMatches + 1 : expectedMatches;
  ...
  getWheelPlayers() {
    ...
    const weekMatches = this.getMatchesForWeek(this.currentWeek);
    const assignedPlayers = weekMatches.reduce(function (arr, m) {
      return arr.concat([m.player1, m.player2]);
    }, [] as string[]);
    const availablePlayers = this.players.filter(
      (p) => !assignedPlayers.includes(p) && p !== "Bye Week"
    );
    ...
    if (players.length % 2 === 1) {
      ...
      const playersNeedingBye = this.getPlayersNeedingBye(
        allActivePlayers,
        availablePlayers
      );
  ...
  getFairByeFromAvailable(availablePlayers: string[]): string {
    ...
    this.roundRobinMatches.forEach((match) => {
      if (
        match.player2 === "Bye" &&
        match.player1 !== "Bye" &&
        availablePlayers.includes(match.player1)
      ) {
        byeCounts[match.player1] = (byeCounts[match.player1] || 0) + 1;
      }
    });
    ...
    // Find players with minimum bye count
    const minByes = Math.min(...Object.values(byeCounts));
    const playersWithMinByes = availablePlayers.filter(
      (player) => byeCounts[player] === minByes
    );
  ...
  nextWeek() {
    this.currentWeek =
      this.currentWeek < this.maxWeeks ? this.currentWeek + 1 : this.maxWeeks;
    this.selectedPlayer = null; // Clear selection when changing weeks
    this.shouldAnimateStatic = false; // Stop static animation
    this.wheelFeedback = ""; // Clear feedback messages
    this.redrawWheelForCurrentWeek(); // Redraw wheel with available players
